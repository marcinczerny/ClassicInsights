...
type EntityNodeData = Node<
  Omit<GraphNodeDTO, "id" | "type"> & { label: string }
>;
/**
...
export function EntityNode({ data }: NodeProps<EntityNodeData>) {
  ...
  return (
    <div
      className={`min-w-[150px] rounded-lg border-2 border-white bg-indigo-500 px-4 py-3 shadow-lg ${bgColor}`}
    >
      <Handle type="target" position={Position.Top} className="!bg-white" />

      <div className="text-center">
        <div className="mb-1 text-xs font-semibold uppercase text-white/70">
          {entity_type}
        </div>
  ...
  return (
    <div className="min-w-[150px] rounded-lg border-2 border-amber-500 bg-amber-50 px-4 py-3 shadow-lg">
      ...
      <div className="text-center">
        ...
        {note_preview && (
          <div className="mt-1 text-xs text-amber-700 line-clamp-2">
            {note_preview}
          </div>
        )}
      </div>
